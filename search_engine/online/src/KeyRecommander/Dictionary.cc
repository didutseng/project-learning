...
//传入两个字符串对象的 常量引用，分别是 lhs和 rhs
{
	size_t lhs_len = length(lhs);
	size_t rhs_len = length(rhs);
	int editDist[lhs_len + 1][rhs_len + 1];
    //editDist[i][j] 表示 lhs[0..i-1] 到 rhs[0..j-1] 的最小编辑距离
	...
	// 处理多字节字符（如中文）
    {
        ...
        //检测当前字符占用的字节数（英文1字节，中文3字节）
		sublhs = lhs.substr(lhs_idx, nBytes);
        //// 提取完整字符
        //按实际字符（而非字节）切分字符串，避免乱码
		lhs_idx += (nBytes - 1);
        // 跳过剩余字节
        //指向下一个要处理的字节，因为lhs_idx会自动+1，所以后面需要-1
